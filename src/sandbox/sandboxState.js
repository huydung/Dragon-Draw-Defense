...
export function updateSelectedPoint(state, pointIndex, point) {
  ...
  const currentPoints = state.templates[state.selectedName];
  const nextPoint = snapPointToNearby(currentPoints, pointIndex, normalizePoint(point));
  const points = currentPoints.map((existingPoint, index) => (index === pointIndex ? nextPoint : existingPoint));
...
function snapPointToNearby(points, ignoredIndex, point) {
  const snapTarget = points.find((candidate, index) => {
    if (index === ignoredIndex) {
      return false;
    }

    return Math.hypot(candidate[0] - point[0], candidate[1] - point[1]) <= GAME_CONFIG.UI.TEMPLATE_POINT_SNAP_DISTANCE;
  });

  return snapTarget ? [...snapTarget] : point;
}
